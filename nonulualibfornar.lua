...
function StringStartsWithIgnoreCase(fullString, partialString)
    ...
    return string.find(fullString, partialString, 1, true) ~= nil
end
...
function Movement(x, y, z, fly, stopDistance)
    LogInfo("[NonuLuaLib] Sarting movement")
    fly = fly or false
    ...
    if GetDistance(destination,Player.Entity.Position) <= stopDistance then
        LogInfo("[NonuLuaLib] Too close to the position (%.3f, %.3f, %.3f)", x,y, z)
        return true
    end
    ...
    if not IPC.vnavmesh.IsReady() then
        LogInfo("[NonuLuaLib] Navmesh not ready")
        return false
    end
    ...
    while IPC.vnavmesh.IsRunning() do
        ...
        checkdisttick = checkdisttick + 1
        if checkdisttick == 8 then
            local pos = Player.Entity.Position
            local dx = oldPos.X - pos.X
            local dy = oldPos.Y - pos.Y
            local dz = oldPos.Z - pos.Z
            local diststuck = math.sqrt(dx * dx + dy * dy + dz * dz)            
            if diststuck < 1 then 
                yield("/gaction jump")
            end
            oldPos = Player.Entity.Position
...
function PathToObject(targetName, fly, stopDistance,maxDistance)
    fly = fly or false
    ...
    if obj then
        ...
        if GetDistance(pos,Player.Entity.Position) <= stopDistance then
            LogInfo("[NonuLuaLib] Find but path too short")
            return true
        end

        if GetDistance(pos,Player.Entity.Position) > maxDistance 
        then
            LogInfo("[NonuLuaLib] Find but path too long")
            return false
        end

...
local map70_794 = {
    ...
    -- Summon 1    
        ...
        { action="/p interactd", checkOk = "%[(%d+):(%d+)%]The summon is dispelled."},
        { pos = Vector3(100.08,0.10,99.97), action="/p interactt", wait=7 },
        { action="cleanBag"},
    ...
    -- Summon 5
        ...
        { pos = Vector3(100.08,0.10,99.97), action="/p interactt", wait=7 },
        { action="cleanBag"},
        { pos = Vector3(100.24,0,83.49), action="/p interactd", wait=2 }
    }
}
...
local map70_712 = {
    ...
    -- Coffre 1
        { os = Vector3(0.082,100.39,299.91), action="/p interactt", checkOk = "%[(%d+):(%d+)%]The First Sluice is no longer sealed!", wait=5 },
        { pos = Vector3(0.082,100.39,299.91), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Door left
        { pos = Vector3(-20.18,100.00,279.70), action="/p interactd", checkOk = "%[(%d+):(%d+)%]The gate to the 2nd chamber opens." ,wait=7 },
    -- Path to coffer 2
        { action="movealittle", wait=5},   
    -- Coffre 2
        { pos = Vector3(-0.098,50.4,179.63), action="/p interactt", checkOk = "%[(%d+):(%d+)%]The Second Sluice is no longer sealed!", wait=5 },
        { pos = Vector3(-0.098,50.4,179.63), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Door left
        { zoneId = 712, pos = Vector3(-20.19,50.00,159.78), action="/p interactd", checkOk = "%[(%d+):(%d+)%]The gate to the 3rd chamber opens." ,wait=7 },
    -- Path to coffer 3    
        { action="movealittle", wait=5},     
    -- Coffre3
        { pos = Vector3(0.02,0.40,60.21), action="/p interactt", checkOk = "%[(%d+):(%d+)%]The Third Sluice is no longer sealed!", wait=5 },
        { pos = Vector3(0.02,0.40,60.21), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Door left    
        { zoneId = 712, pos = Vector3(-20.20,0.00,39.73), action="/p interactd", checkOk = "%[(%d+):(%d+)%]The gate to the 4th chamber opens." ,wait=7 },   
    -- Path to coffer 4    
        { action="movealittle", wait=5}, 
    -- Coffre4
        { pos = Vector3(0.04,-49.59,-60.14), action="/p interactt", checkOk = "%[(%d+):(%d+)%]The Fourth Sluice is no longer sealed!", wait=5 },
        { pos = Vector3(0.04,-49.59,-60.14), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Door left
        { pos = Vector3(-19.75,-49.89,-79.70), action="/p interactd", checkOk = "%[(%d+):(%d+)%]The gate to the 5th chamber opens." ,wait=7 },
    -- Path to coffer 5
        { action="movealittle", wait=5} ,
    -- Coffre5
        { pos = Vector3(-0.45,-99.59,-179.02), action="/p interactt", checkOk = "%[(%d+):(%d+)%]The Fifth Sluice is no longer sealed!", wait=5 },
        { pos = Vector3(-0.45,-99.59,-179.02), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Door left
        { pos = Vector3(-20.40,-99.99,-200.06), action="/p interactd", checkOk = "%[(%d+):(%d+)%]The gate to the 6th chamber opens." ,wait=7 },       
    -- Path to coffer 6
        { action="movealittle", wait=5},
        { pos = Vector3(-0.06,-149.59,-299.76), action="/p interactt", checkOk = "%[(%d+):(%d+)%]The Sixth Sluice is no longer sealed!", wait=5 },
        { pos = Vector3(-0.06,-149.59,-299.76), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Door left
        { pos = Vector3(-20.45,-149.99,-320.02), action="/p interactd", checkOk = "%[(%d+):(%d+)%]The gate to the final chamber opens." ,wait=7 },       
    -- Path to coffer 6
        { action="movealittle", checkOk = "%[(%d+):(%d+)%]The Sevventh Sluice is no longer sealed!", wait=5},   
    -- Coffre7
        { pos = Vector3(-0.18,-199.60,-420.02), action="/p interactt", wait=5 },
        { action="cleanBag"},
    -- Exit
        { pos = Vector3(0.08,-200.00,-436.21), action="/p interactd"}       
    }
}
